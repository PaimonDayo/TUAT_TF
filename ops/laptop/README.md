...
現コードはサーバーとブラウザの両方からSupabaseへ接続するため、Vercelの環境変数だけでなくブラウザからも到達可能なHTTPS APIの入口が必要。通常版のURL/キーを切り替えて再ビルドする。本人限定のPC試験フラグ・ゲートをそのままVercelへ移す構成ではない。

次の段階は、安定したAPI公開先、Google OAuth/既存ユーザーの再ログイン、画像・同期・通知の接続を整え、Vercelの別PreviewでPCバックエンドを検証する。現在のQuick TunnelはランダムURLで開発・試験専用のため常用先にはしない。アプリの既存vercel.app URL自体に独自ドメインは不要だが、PC APIの安定した入口は別途必要。PC停止中はVercelが稼働していてもデータ機能が止まる。

この確認では本番の接続先を変更していない。切替時は下記の最終バックアップ・書き込み停止・切り戻し手順を実施する。
