## 2026-09-23 プログラムの途中経過による出場者欠落（本番反映待ち）

Codex: 男子走幅跳の未試技2人が公式途中経過から一時的に消え、同期で空名簿へ置換される原因を確認。競技中の個人種目は既知の出場者を結果待ちで保持する修正と、種目一覧/詳細欠落の取込拒否を実装。全56種目と開始前バックアップを照合し33種目74出場・欠落0を確認。全403テスト（別作業監視4件込み）・対象eslint・TypeScript付きPC本番設定build成功。自動承認レビューがmasterへのpushによる本番デプロイを明示承認不足として拒否。コードと本番データの反映は未実施。詳細: docs/PROGRAM-2026-09-23.md。
