...
症状: アプリの記録フォームで感想欄（memo）に入力しても、スプレッドシート側に反映されない。
**切り分け済み（2026-07-09 Claude Fable 5、静的解析）:**
- マッピング自体は存在する（`sheet-sync.ts` BUILTINS の `memo` → 見出しキーワード「感想」「コメント」）。
- `record_source='app'` の部員なら push 経路（appToCellsFull → gasPost writeCells）で memo も送られるはずで、updated_at 自動更新トリガー（20260623100000）も健在。
- **最有力の真因: 該当部員が `record_source='sheet'`（pull専用）で、アプリ→シート方向がそもそも存在しない**（タスク2の方向固定仕様の帰結）。この場合の恒久修正はタスク16のwrite-through。
- 調査手順: ①該当部員の `profiles.record_source` を確認 → 'sheet' ならタスク16と一体で解決。② 'app' なのに反映されないなら実バグ: 該当記録の `updated_at`/`synced_at` の大小、sheet_sync_runs の失敗記録、シート側の実際の見出し名（「感想」を含むか）の順に確認。cron（毎時）待ちを「反映されない」と誤認していないかも確認。
...
- 2026-07-09 / Claude Code (Fable 5) / オーナー指示によりバックログにタスク15（感想欄がスプシに反映されないバグ・キュー最前列）とタスク16（'sheet'部員のハイブリッド入力＝write-through、旧TF方式・部員100人想定）を追記。着手順にも最前列の注記を追加。タスク2の「'sheet'部員は編集不可」は16が上書き。コード変更なし → (このcommit)
