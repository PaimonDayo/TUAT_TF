- 2026-08-29 / Claude Opus 5 / アプリの返信を削除しても、スプレッドシートに残った写しが「スプレッドシートからの返信」として復活する問題を修正。原因: 記録の読み取りは全て公開CSV（fetchPublicMember）経由で、GASが書き込み時に付けるセルのメモ（TUAT_APP_COMMENT:<id>）を見られないため、二重表示の判定を「今あるアプリ返信の本文と列位置」だけに頼っていた。返信を削除するとその手掛かりごと消えるので、次の同期でシートのセルが新規の返信として取り込まれていた（編集は sheet_reply_index が残るので無事だった）。migration 20260829150000 で sheet_reply_tombstones（削除された返信の列位置と書き出し本文）を追加し、comments の AFTER DELETE トリガーで痕跡を残す＋既に復活していた sheet_record_replies の行も同時に削除。取込側（reconcileSheetReplies＝毎時cron・手動同期・マイページの即時反映の3経路すべてが通る）で痕跡を除外する。痕跡は列位置と本文の両方で照合するので、投稿者が改名していても、列位置が未記録の古い返信でも当たる。対象は連携者の記録へのコメントだけ（つぶやき・非連携者は対象外）。practice_records への外部キーは張っていない（記録ごと削除したときのカスケード順に依存するため）。ローカルPostgres 16に最小スキーマを立ててmigrationを実際に適用し、6ケース（列位置あり/なし・つぶやき除外・非連携者除外・記録カスケード・部員カスケード）で挙動を確認済み。**この修正より前に復活してしまった行は痕跡が無いため自動では消えない（シート側のセルを消すのが正）。** 全216テスト・tsc・対象eslint・build成功。**migration は本番未適用・実機確認は未実施**。シートのセル自体を消す案（GASに clearReply を足してメモから該当セルを特定）は未実施＝要判断 -> (this commit)
- 2026-08-29 / Claude Opus 5 / コード改善6件＋お知らせの通知先修正。①スレッド・フォルダ・記事のピン留めが生の update で RLS 拒否を無言成功にしていたのを safeUpdate 経由へ（safeUpdateMessage に操作名の引数を追加）②created_at のタイブレークが getFeed にしか無く、スプシ由来記録（created_at＝練習日0時で同値多発）の並びがタイムラインとマイページで食い違っていたのを src/lib/feed-sort.ts に共通化③Block の妥当性判定3箇所を ALL_BLOCKS/isBlock へ集約（予定スプシ側は SCHEDULE_SHEET_BLOCKS として現状維持＝マネージャー除外は意図的）④attachTweetSocialData の投稿×選択肢の総当たりを tweet_id で束ねる形へ⑤getHomeNotices が有効なお知らせ全件を読んでいたのを「重要は全件・通常は50件・明日締切は別クエリ」に分割⑥\uXXXX エスケープの日本語を18ファイルで実体化（機械検証済み: HEAD をデコードすると新ファイルと完全一致）。**お知らせの通知先にマネージャーを追加**: mentioned_blocks の CHECK が middle_long/short/jump/throw のみで、通知トリガーは profiles.blocks との配列重なり判定のため blocks=['manager'] の部員はブロック指定では絶対に通知が届かなかった（migration 20260829140000 で 'manager' を追加）。RecipientPicker を中長距離／短距離／マネージャーの3択にし、画面の「通知対象◯人」を DB と同じ完全一致へ（従来は matchSimpleBlock 経由で中長・短の指定にマネージャーを混ぜて数えており、実際には通知されない人まで人数に入っていた）。あわせて、お知らせ編集時に mentioned_blocks を normalizeProfileBlocks（部員1人を1節に畳む用途）に通していたため中長＋短の指定が編集のたび片方に減っていたのを normalizeAudienceBlocks で修正。練習中止のお知らせは予定の対象ブロックを引き継ぐ別経路のためマネージャー除外のまま（要判断）。全212テスト・tsc・eslint（警告0）・build成功。**migration は本番未適用・実機確認は未実施** -> (this commit)
