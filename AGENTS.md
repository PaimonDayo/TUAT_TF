...
- **切替時リコンサイル（オーナー確定 2026-07-04・必須）**: record_source を切り替えた瞬間に、その部員だけを対象に一度だけ「両方を揃える同期」を自動実行してから方向を固定する。app→sheet: アプリの中身のある項目をシートへ書き出してから切替。sheet→app: シートの中身のある項目をアプリへ取り込んでから切替。いずれも非破壊（空で相手を消さない）。これにより「切替で宙に浮く/古い側に上書きされるデータ」を無くす。失敗したら切替自体を中止してエラー表示。
