...
>
> **【2026-07-09 オーナー指示・キュー最前列】未消化タスクより先に `16`（練習記録のスプシDB化）を実施する。**（旧タスク15＝感想欄未反映バグは、真因「タブに感想見出しが無い」をオーナーがシート側で解消したため削除済み 2026-07-09。「書けなかった項目の可視化」はタスク16の一項目として残る）
...
- **スプシメイン部員の読み**: 本人の記録閲覧・編集画面は、その部員のタブを**CSVエクスポート直読み**（`/export?format=csv&gid=<gid>`。APIキー不要・「リンクを知っている全員が閲覧可」で動作、2026-07-09実測済み。タブ一覧gidは `/htmlview` から抽出、実装例は旧TF `sheetsService.js`）。タブ単位なので軽く、表示時点で常に最新。共有設定変更で読めなくなった場合は GAS fetchAllRaw へフォールバック。
- **スプシメイン部員の書き**: アプリからの入力・編集は GAS `writeCells` で**その場でシートの同日行へ直接書き込む**（write-through）。シートの既存値をアプリから上書きしてよい（オーナー確認済み）。ただし**空の項目でシートのセルを消さない**（非破壊＝厳守ルール）。書き込み成功＝保存成功。失敗したら保存エラーとして表示（Supabaseだけ更新して「保存できた」と見せない）。
- **Supabaseミラー（内部配管・必須）**: いいね・コメントは practice_records の行IDに紐づき、タイムライン・週間集計・ランキングもSupabase集計のため、スプシメイン部員の記録も**「写し」行をSupabaseに維持する**（from_sheet=true の現行pullを流用）。更新タイミング: 毎時cron＋本人記録の閲覧時＋write-through直後。ミラーはスプシ→Supabaseの一方向生成で、ミラー側の都合でスプシを消す・書き換えることは絶対にしない。部員には「スプシが本体」とだけ見える。
- **既存のRecordFormブロック撤去**: タスク2の「'sheet'部員は編集・新規作成不可（閲覧のみ）」を撤去し、通常の編集画面に戻す（読み取り専用表示 e9093c6 も撤去）。タスク2の当該記述は**本節が正**（上書き）。
- **1日1記録の制約**: スプシメイン部員はシート行と1:1対応のため、アプリからの同日2件目は「同日の既存記録の編集」へ誘導。
- **未マップ項目の可視化**: GAS writeCells に「渡された見出しのうち書けなかったもの」を返させ、保存時・同期結果に表示する（見出し欠落タブで項目が黙って落ちるのを防ぐ。旧タスク15の教訓）。
- **切替時リコンサイル（タスク2）・SYNC_CUTOFF・未来日除外・dryRun・sheet_sync_runs は維持**。本番初回前に厳守ルールどおりスナップショット＋dryRunで差分確認。
- **100人規模の注意**: htmlviewのgid抽出結果はキャッシュ。毎時cronのミラー更新は fetchAllRaw の応答肥大・GAS実行時間（6分制限）を実測し、必要ならCSV直読みの並列取得へ置換またはバッチ分割。
