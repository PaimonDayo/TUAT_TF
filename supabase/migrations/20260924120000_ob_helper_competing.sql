...
BEGIN
  ...
  IF btrim(p_assignment)<>'' AND EXISTS(
    SELECT 1 FROM public.ob_meet_entries e CROSS JOIN LATERAL unnest(e.events) event
    JOIN public.ob_duty_event_slots s ON s.slot_time=p_slot_time AND s.event_name=CASE substring(event FROM 3) WHEN '立ち五段' THEN '立ち五段跳び' ELSE substring(event FROM 3) END
    WHERE e.meet_key='ob-2026' AND e.profile_id=p_profile_id
