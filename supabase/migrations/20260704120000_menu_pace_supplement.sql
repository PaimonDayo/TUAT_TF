-- 練習メニューに詳細(content)とは別枠の「補強」「ペース」を追加。
-- 中長距離向けにCSV一括登録でも埋められるようにするための構造化フィールド。
...
-- save_practice_menu: pace/supplement パラメータを追加。
-- content が空でも pace/supplement のどちらかがあれば保存できるよう必須条件を緩和。
DROP FUNCTION IF EXISTS public.save_practice_menu(UUID, TEXT, TEXT, TEXT, UUID[], UUID);
...
BEGIN
  ...
  IF clean_content = '' AND clean_pace IS NULL AND clean_supplement IS NULL THEN
