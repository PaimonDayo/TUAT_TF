-- Pull spreadsheet-entered records once per day at 00:00 JST (15:00 UTC).
-- App-entered records continue to use the immediate /api/sheets/push-record path.
DO $$
DECLARE
  sync_job_id BIGINT;
BEGIN
...
-- A daily invocation must cover the whole expected club size. The previous
-- function-level cap of 30 would otherwise leave later members until another day.
