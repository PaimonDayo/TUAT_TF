...
export default async function ObEntriesPage() {
  ...
  const { entries, members, history, party, duties } = await getObEntries();
  return <><SubHeader title="OB戦エントリー" backHref="/mypage" />
    <ObEntryReview initial={entries.data ?? []} members={members.data ?? []} viewerId={profile.id} party={party.data ?? []} duties={duties.data ?? []}
