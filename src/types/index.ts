...
/** 招待への応答。未回答（pending）と明示辞退（declined）は区別して保持する。 */
export type ItoInvitationStatus = "pending" | "joined" | "declined";
export type ItoParticipantStatus = "active" | "excluded";
/** 得点の出どころ。1ラウンドで1人が受け取るのはどちらか一方。 */
export type ItoPointSource = "group" | "leader_team";
...
export interface ItoInvitation {
  id: string;
  game_id: string;
  profile_id: string;
  /** このラウンドから参加してもらう想定（初回は 1） */
  round_no: number;
  status: ItoInvitationStatus;
  invited_by: string | null;
  invited_at: string;
  responded_at: string | null;
}

export interface ItoParticipant {
  game_id: string;
  profile_id: string;
  status: ItoParticipantStatus;
  joined_round: number;
  left_round: number | null;
  updated_at: string;
}

export interface ItoRound {
  id: string;
  game_id: string;
  round_no: number;
  phase: ItoPhase;
...
export interface ItoGroup {
  id: string;
  round_id: string;
  name: string;
  /** 代表者チーム。所属行は持たず、is_leader のメンバーから導出する。 */
  is_leader_team: boolean;
  sort_order: number;
}

export interface ItoGroupMember {
  id: string;
  round_id: string;
  group_id: string;
  profile_id: string;
...
/** 自分の秘密数字。他人の分は結果発表までサーバーから返らない。 */
export interface ItoSecret {
  round_id: string;
  profile_id: string;
  number: number;
  assigned_at: string;
...
/** 管理者向けの配布状況。数字そのものは含まない。 */
export interface ItoSecretStatus {
  profile_id: string;
  assigned: boolean;
  confirmed: boolean;
}

export interface ItoLeaderAnswer {
  round_id: string;
  profile_id: string;
  answer: string;
...
/** グループ共有の並び順。revision で競合を検出する。 */
export interface ItoGroupOrder {
  group_id: string;
  round_id: string;
  /** 上（先頭）ほど数字が大きい、代表者の profile_id の並び */
  order_ids: string[];
  revision: number;
  submitted: boolean;
  updated_by: string | null;
...
export interface ItoRoundScore {
  group_id: string;
  round_id: string;
...
export interface ItoPointEvent {
  id: string;
  game_id: string;
  round_id: string;
  profile_id: string;
