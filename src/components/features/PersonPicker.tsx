...
export function PersonPicker({ people, value, onChange, label = "対象者", excludeIds = [], includedIds = [] }: {
  ...
  const filtered = useMemo(() => people.filter((person) => {
    ...
    return (!q || person.display_name.toLowerCase().includes(q)) && (blocks.length === 0 || (person.blocks ?? []).some((block) => blocks.includes(block))) && (grades.length === 0 || grades.includes(person.grade ?? ""));
