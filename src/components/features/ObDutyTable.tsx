...
export function ObDutyTable({entries,members}:{entries:ObEntry[];members:EntryMember[]}) {
  ...
  function download() {
    const values=[
      ...
      ["学年","氏名","本人照合",...OB_DUTY_SLOTS.map((s)=>`${s.time} ${s.label}`),"補助員担当（記入用）"],
      ...rows.map((row)=>[row.grade,row.name,row.linked?"アプリ名簿":"未照合",...OB_DUTY_SLOTS.map((slot)=>dutyCell(row.entry,slot)),""]),
    ];
  ...
  return <div className="space-y-4"><Card className="space-y-2 p-4"><h2 className="text-headline">補助員検討表</h2>
    <p className="text-caption">開始時刻ごとの出場登録です。アップ・移動・競技終了時刻を確認して担当を決めてください。「出場登録なし」は空き時間の確定ではありません。リレーは当日確認です。</p>
    <p className="text-caption">未照合の回答は別行です。同じ部員が名簿と回答の両方に出る場合は「本人照合」で紐付けてください。</p>
  </Card>
  ...
  <p className="text-caption">{rows.length}行・横にスクロールできます。CSVには担当を書き込む欄があります。</p>
  <Card className="overflow-hidden"><div className="max-h-[65dvh] overflow-auto" tabIndex={0} role="region" aria-label="部員別の出場予定表">
    <table className="w-full min-w-[1400px] border-collapse text-left text-[13px]"><caption className="sr-only">補助員検討用の出場予定</caption>
      ...
      <tbody>{rows.map((row)=><tr key={row.id} className="border-t border-separator"><th scope="row" className="sticky left-0 z-10 bg-card p-3 font-normal"><span className="block text-caption">{row.grade}{!row.linked?"・未照合":""}</span>{row.name}</th>
        {OB_DUTY_SLOTS.map((slot)=>{const value=dutyCell(row.entry,slot);const competing=row.entry?.events.some((e)=>slot.events.includes(e.slice(2)));return <td key={slot.time} className={`border-l border-separator p-3 ${competing?"bg-accent/10 font-medium text-accent":"text-muted2"}`}>{value}</td>;})}
      </tr>)}</tbody>
    </table>
  </div></Card></div>;
}
