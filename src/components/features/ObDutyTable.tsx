  ...
  <Card className="overflow-hidden"><div className="max-h-[65dvh] overflow-auto" tabIndex={0} role="region" aria-label="部員別の出場予定表">
    <table className="w-full min-w-[1400px] border-collapse text-left text-[13px]"><caption className="sr-only">補助員検討用の出場予定</caption>
      ...
      <tbody>{rows.map((row)=><tr key={row.id} className="border-t border-separator"><th scope="row" className="sticky left-0 z-10 bg-card p-3 font-normal"><span className="block text-caption">{row.grade}{!row.linked?"・未照合":""}</span>{row.name}</th>
        {OB_DUTY_SLOTS.map((slot)=>{const value=dutyCell(row.entry,slot);const competing=!!row.entry?.events.some((e)=>slot.events.includes(e.slice(2)));const duty=row.linked?findDuty(row.id,slot.time):undefined;return <td key={slot.time} className={`relative border-l border-separator align-top ${competing?"bg-accent/10":""}`}>
          {row.linked ? <div className="min-h-24 p-3">
            ...
            <button type="button" aria-label={`${row.name}の${slot.time}の補助員担当`} className="absolute inset-0 h-full w-full cursor-pointer hover:bg-accent/5 focus-visible:outline-2 focus-visible:outline-accent focus-visible:-outline-offset-2"
