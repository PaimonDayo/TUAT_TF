...
/** アプリを開いたとき、1日1回だけOB戦のエントリー確認を促す。閉じればその日はもう出さない。 */
export function ObEntryPrompt({ hasEntry, today }: { hasEntry: boolean; today: string }) {
  // サーバー描画では出さず、端末の記録を読んでから判定する（保存できない端末では出さない）。
  const shownToday = useSyncExternalStore(() => () => {}, () => { try { return localStorage.getItem(KEY) === today; } catch { return true; } }, () => true);
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[60] w-[calc(100%-32px)] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-[18px] bg-card p-5 shadow-xl outline-none">
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            ...
            <Button asChild><Link href={`${OB_PROGRAM_PATH}?edit=mine`} onClick={close}>{hasEntry ? "確認する" : "エントリーする"}</Link></Button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
