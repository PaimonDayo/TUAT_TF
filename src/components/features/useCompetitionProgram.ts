...
export function useCompetitionProgram(competition: CompetitionRow, initialEntries: CompetitionProgramEntryRow[]) {
  ...
  const { id, starts_on: startsOn, ends_on: endsOn } = competition;
  ...
  useEffect(() => startCompetitionProgramPolling(
    ...
    () => { const today = jstToday(); return hasSource && startsOn <= today && today <= (endsOn ?? startsOn); },
  ), [id, startsOn, endsOn, hasSource, initialEntries]);
