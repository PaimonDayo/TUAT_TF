...
> **2026-09-04 現行運用:** アプリで保存した練習記録はその場でスプレッドシートへ書き込む。スプレッドシートで入力・変更した記録のアプリへの取り込みは、毎日0:00 JSTの1回だけ実行する。画面表示時の取り込みと通常UIからの手動全件取り込みは廃止済み。以下の毎時同期に関する記述は旧設計の履歴として残す。

> 前提: `origin/master` が正・force-push禁止・作業前 pull。マイグレーションは新タイムスタンプ＆冪等。
> これは「予定の一括入力」とは**別機能**。あちらはアプリ独自フォーマットの新規スプシ（実装済み。設計メモは2026-09-13に削除した）。
