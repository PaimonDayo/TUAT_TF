...
## 2026-09-10 PC本番への適用結果（以下の未実施記録に優先）

- 現在の本番DBは所有者PC。migration `20260909010000` とPB/UBの独立性を修正する `20260910010000` を適用済み。元の列と29件の既存記録を保持。
- 暗号化バックアップ・復号検証・全対象スナップショット後、別DBで正規化と権限を検証。PC本番の18件に構造化した記録値を補完し、うち1件の種目名を正規化。元の `record`、PB/UB、大学区分は変更していない。
- 自動変換できない11件とPB/UB重複4組は判断を要するため保持。詳細は本番PCの `.contingency/backend/pb-normalization-pc-migrated.review.json`（Git追加・ログ出力禁止）。大学以前かどうかの自動判定はしていない。
- 本番直前のSQLが別DB検証済みSQLとSHA256一致。SQLは全行の変更前・変更後を照合し、差異があれば全体ロールバック。古い計画の再実行拒否も検証済み。
- 旧クラウドのmigration試行は接続切断でロールバック。未適用・書き込み凍結を読み取り確認。所有者の指示によりクラウド更新は後回しとし、以降は追加適用していない。9/17以降は別PCのサーバーへ移行予定。
- 検証: 既存222テスト＋正規化3テスト、中継/暗号化2テスト、TypeScript、対象ESLint、PC本番設定build成功。別DBで本人保存・他人の編集拒否・既存管理者編集・匿名非公開、PB/UBの独立性、負数拒否を確認。iOS実機は未確認。
