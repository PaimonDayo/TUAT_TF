# 通知機能 実装仕様（担当: Antigravity）

最終更新: 2026-06-21 / 起案: Claude Code（設計確定済み）

> このドキュメントが実装の「正」。着手前に `AGENTS.md` の厳守ルールと「作業ログ」運用を必ず確認すること。
...
- **通知種別は3つ＋ユーザーが受信ON/OFFを選べる**：
  - `comment` … 自分の投稿（練習記録・つぶやき）へのコメント
  - `schedule_update` … 予定の追加・変更
  - `notice` … お知らせの投稿
  - 既定は**全てON**。マイページに3種のトグル。OFFの種別は通知行を作らない＆Pushも送らない。
