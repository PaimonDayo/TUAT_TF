## 2026-09-13 いまの状態（ここが最新）

- **本番はVercel＋所有者PCのWSL内Supabase**（下の2026-09-09節が構成の説明）。所有者は
  旧クラウドへの復帰を後回しにし、**9/17以降に別のPCをサーバーにする**方針。
  その手順は **`ops/laptop/SERVER-HANDOFF.md`**（2026-09-13追加）。
- 巨大化していた `lib/queries.ts`（1183行）と `lib/sheet-sync.ts`（1312行）を
  ドメイン別モジュールへ分割した。**入口は `@/lib/queries` / `@/lib/sheet-sync` のまま**なので
  呼び出し側の書き方は変わらない。続きの計画は `docs/ARCHITECTURE-REFACTOR-PLAN.md`。
- タブ切替が遅い件は、**DBへの往復を直列に積んでいた**のが主因。`getCurrentUserId()`（DBを使わない）を
  足して、プロフィール取得と各画面の取得を同時に投げるようにした。下タブの先読みも戻した。
  規約は `docs/UI-UNIFICATION.md` §4.5。
- 押下フィードバックを `pressable` 1種に統一（同 §8）。
- **バックエンドのプロセスを再起動するまで、ローカルのバックアップ自動削除は効かない**
  （稼働中の `run-backend.mjs` が古いコードを持っているため）。`ops/laptop/restart-backend.ps1`。
- 未実施: master反映・Vercel Production確認・iOS実機確認。

## 2026-09-09 PC本番運用（構成の説明）
...
- 当時の指示書（`TASK-codex.md` / `QA-CHECKLIST.md` / `UX-ISSUES-2026-06.md` ほか）は2026-09-13に削除した。履歴はGitにある。
- `UI-UNIFICATION.md` に残る未チェック項目も確認観点であり、現在の実装指示ではない。
